...
export default async function () {
  ...
  const response = await contract.greetSomeone("Alice");
  console.log(`Current message is: ${response}`);

  // Run contract write function
  const transaction = await contract.setGreeting("Aloha");
  ...
  console.log(`The message now is: ${await contract.greetSomeone("Alice")}`);
}
